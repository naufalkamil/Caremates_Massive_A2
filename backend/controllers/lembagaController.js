...
const createNewLembaga = (req, res) => {
    const{nama, alamat, nomor_telepon, website, media_sosial, nomor_akta, nomor_izin, dokumen_resmi, logo, foto_sampul, jumlah_tanggungan, visimisi, detail } = req.body;
    if(!nama || !alamat || !nomor_telepon || !website || !media_sosial || !nomor_akta || !nomor_izin || !jumlah_tanggungan || !visimisi || !detail){
        return res.status(400).json({
    ...
    database.query(
        `INSERT INTO lembaga (nama, alamat, nomor_telepon, website, media_sosial, nomor_akta, nomor_izin, dokumen_resmi, logo, foto_sampul, jumlah_tanggungan, visimisi, detail) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,)`,
        [nama, alamat, nomor_telepon, website, media_sosial, nomor_akta, nomor_izin, dokumen_resmi, logo, foto_sampul, jumlah_tanggungan, visimisi, detail],
        (err, results) => {
          if (err) {
            console.error(err);
            return res.status(500).json({
...
const updateLembagaById = (req, res) => {
    const {id} = req.params;
    const { nama, alamat, nomor_telepon, website, media_sosial, nomor_akta, nomor_izin, dokumen_resmi, logo, foto_sampul, jumlah_tanggungan,visimisi, detail } = req.body;
    if ( !id || !nama || !alamat || !nomor_telepon || !website || !media_sosial || !nomor_akta || !nomor_izin || !dokumen_resmi || !logo || !foto_sampul || !jumlah_tanggungan || !visimisi || !detail ) {
        return  res.status(400).json({
    ...
    database.query(`UPDATE lembaga SET nama = ?, alamat = ?, nomor_telepon = ?, website = ?, media_sosial = ?, nomor_akta = ?, nomor_izin = ?, dokumen_resmi = ?, logo = ?, foto_sampul = ?, jumlah_tanggungan = ?, visimisi = ?, detail = ? WHERE id = ?`, [nama, alamat, nomor_telepon, website, media_sosial, nomor_akta, nomor_izin, dokumen_resmi, logo, foto_sampul, jumlah_tanggungan, visimisi, detail, id], (err, results) => {
        if(err) {
            console.error(err);
            res.status(500).json({
